...
class u32_dtc_api final {
 public:
  ...
  /**
* @brief Request 0x19, Subfunction 0x04
* Gets matching snapshots for DTC

* @param tx address of the server
* @param dtc DTC for which snapshots need to be retrieved
* @param id id to match snapshots
* @throw uds::exceptions::uds_invalid_argument if given dtc is invalid
* @return response containing wanted snapshot if request was successful, specific error or negative
* response otherwise
*/
  DLL_PUBLIC uds::response::uds_client_response<freeze_frames_for_dtc> get_snapshots_for_dtc_by_id(
      uint32_t tx, uint32_t dtc, uint8_t id);  // read dtc, sub 0x4, lower
   ...
   */
  DLL_PUBLIC uds::response::uds_client_response<std::map<uint8_t, extended_data_record>> get_extended_data_for_dtc(
      uint32_t tx, uint32_t dtc,
