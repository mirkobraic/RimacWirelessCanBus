...
class client_configuration_manager {
 public:
   ...
   */
  virtual std::map<std::pair<T, T>, extended_data_record_id_to_length> get_extended_data_read_instructions();
...
template <typename T>
std::map<std::pair<T, T>, extended_data_record_id_to_length>
client_configuration_manager<T>::get_extended_data_read_instructions() {
  return std::map<std::pair<T, T>, extended_data_record_id_to_length>();
}
