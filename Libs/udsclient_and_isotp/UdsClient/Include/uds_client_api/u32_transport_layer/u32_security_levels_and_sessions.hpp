...
class u32_security_levels_and_sessions final {
 public:
  ...
  DLL_PUBLIC uds::response::uds_client_response<uds::response::positive_response> send_switch_to_session(
      uint32_t tx, uds::session session);
   ...
   */
  DLL_PUBLIC uds::response::uds_client_response<uds::session> get_active_session(uint32_t tx);

  /**
   * @brief Request 0x27, Subfunction N/A
   * Requests the server to switch to level using murmurhash for calculating key from seed
   * @param tx address of the server
   ...
   * @return response indicating successful request, specific error or negative response otherwise
   */
  DLL_PUBLIC uds::response::uds_client_response<uds::response::positive_response> send_switch_to_level_with_murmurhash(
      uint32_t tx, int level);
  ...
  send_switch_to_level_with_xcp_algorithm(uint32_t tx, int level);
  ...
  DLL_PUBLIC uds::response::uds_client_response<uds::response::positive_response> send_switch_to_level_with_dll(
      uint32_t tx, int level, const std::string& dll_path);
#endif
