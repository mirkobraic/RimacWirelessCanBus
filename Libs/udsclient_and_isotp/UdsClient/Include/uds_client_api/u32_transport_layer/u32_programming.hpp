...
class u32_programming final {
 public:
   ...
   * checksum)
   * @param progress_callback Callback to be called with updated active percentage of each phase
  ...
  DLL_PUBLIC uds_programming_result program_flash_memory(
      ...
      const std::vector<ecu_memory_segment>& flash_segments, uint8_t switch_to_bootloader_sleep_period_in_seconds = 3,
      uint32_t routine_control_calls_timeout_millis = 5000, std::function<void(float)> progress_callback = nullptr,
      std::function<void(const std::string&)> progress_phase_callback = nullptr);
   ...
   * @return response indicating successful request, specific error or negative response otherwise
   */
  DLL_PUBLIC uds_programming_result
  program_flash_memory(uint32_t tx, const std::optional<std::vector<flash_memory_sector>>& clear_sectors_opt,
